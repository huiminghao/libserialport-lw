...
SP_API enum sp_return sp_open(struct sp_port *port, enum sp_mode flags)
{
    ...
    SetupComm(port->hdl, 16000, 16000);
                                           ...
                                           const void *buf, size_t count)
{
    ...
    if(count > sizeof(port->pending_byte)) {
        count = sizeof(port->pending_byte);
    }
    memcpy(port->pending_byte, buf, count);

    /* Start asynchronous write. */
    if (WriteFile(port->hdl, port->pending_byte, count, NULL, &port->write_ovl) == 0) {
